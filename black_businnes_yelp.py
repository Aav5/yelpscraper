...
fieldnames = ['name', 'address', 'feature', 'reviews', 'stars', 'self-identified', 'delivery', 'takeaway']
...
for restaurant in restaurants:
    ...
    if (check_mark and delivery_mark) in delivery_takeaway_wrap:
        gives_delivery = 1
    else:
        gives_delivery = 0
    delivery.append(gives_delivery)
    if (check_mark and takeaway_mark) in delivery_takeaway_wrap:
        gives_takeaway = 1
    else:
        gives_takeaway = 0
...
# loops through the entire page finding black-owned restaurant and passes the boolean result into self_identified list
# repeats the same as the url changes
for pages in all_url:
    list_of_restaurant = soup.find('div', class_=' raw__09f24__3Obuy')
    string_list_of_restaurant = str(list_of_restaurant)
    if string_list_of_restaurant == "black-owned":
